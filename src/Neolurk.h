...
class Neolurk {
private:
    std::string api_base = "https://neolurk.org/w/api.php";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("neolurk.org"));
...
public:
    ...
    pplx::task<json::value> search(const std::string& query,int limit=20,int offset=0) {
        return make_api_call("?action=opensearch&format=json&formatversion=2&search=" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(query))) + "&limit=" + std::to_string(limit) + "&namespace=" + std::to_string(offset),"GET");
    }

    pplx::task<json::value> get_page(const std::string& title) {
        return make_api_call("?action=query&format=json&prop=info%7Cextracts%7Cpageimages%7Crevisions%7Cinfo&formatversion=2&redirects=true&exintro=true&exchars=525&explaintext=true&exsectionformat=plain&piprop=thumbnail&pithumbsize=480&pilicense=any&rvprop=timestamp&inprop=url&titles=" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(title))),"GET");
    }
};
